...
export class AscetClassTreeProvider
  implements vscode.TreeDataProvider<AscetClassItem>
{
  ...
  getChildren(element?: AscetClassItem): AscetClassItem[] {
    ...
    if (!this.treeRoot) {
      return [
        new AscetClassItem(
          "$(refresh)  Nhấn Refresh để tải danh sách class",
          true
        ),
      ];
    }
  ...
  async refresh(token?: vscode.CancellationToken): Promise<void> {
    ...
    const result = await runAscetCli<AscetTreeRoot>(
      "list_tree",
      [],
      token
    );
    ...
    if (result.success && result.data) {
      ...
      log(`[Tree] Loaded ${count} ASCET classes.`);
    } else {
