// src/cli/runner.ts — spawn ascet_cli and parse JSON stdout
...
export function runAscetCli<T>(
    ...
    return new Promise((resolve) => {
        ...
        const version = getConfig<string>('ascetVersion', 'auto');
        ...
        proc.on('close', (code) => {
            ...
            try   { resolve(JSON.parse(raw) as CliResult<T>); }
            catch { resolve({ success: false, error: 'CLI output is not valid JSON.', detail: raw.slice(0, 400) }); }
        });
