...
async function handleAnalyze(stream, class_path, token, request) {
    ...
    if (ctx.calc_code) {
        ...
        stream.markdown(`> ⚠️ No calc code loaded${ctx.warning ? `: ${ctx.warning}` : "."}\n\n`);
    }
...
async function handleDiagram(stream, class_path, _token) {
    ...
    const [renderRes, logicRes] = await Promise.all([
        (0, runner_1.runAscetCli)("render_diagram", [
            "--path", class_path,
            "--format", "svg",
        ]),
    ...
    stream.markdown(`> SVG diagram ready. Click below to open in a panel.\n`);
    stream.button({
    ...
    if (logicRes.success && logicRes.data?.netlist?.length) {
        ...
        stream.markdown(nl
            .slice(0, 20)
            .map((e) => `- \`${e.from}\` → \`${e.to}\`${e.signal ? ` *(${e.signal})*` : ""}`)
            .join("\n"));
        if (nl.length > 20) {
...
async function handleAiReview(stream, rawArgs, token) {
    const MODES = ["conservative", "severity", "majority"];
    let mode = "severity";
    let noRag = false;
    ...
    if (!class_path) {
        stream.markdown("❌ Usage: `/ai [conservative|severity|majority] [--no_rag] <class_path>`");
        return;
    }
    stream.progress(`Running full AI review: \`${class_path}\` [mode=${mode}${noRag ? ", no_rag" : ""}]…`);
    (0, logger_1.logInfo)(`/ai review: ${class_path} mode=${mode} no_rag=${noRag}`);
    const cliArgs = [
        class_path,
        "--mode", mode,
        ...(noRag ? ["--no_rag"] : []),
    ];
    const result = await (0, runner_1.runAscetCli)("ai_review", cliArgs, token);
    if (!result.success || !result.data) {
        ...
        for (const issue of d.rule_issues) {
            const icon = issue.severity === "error"
                ...
                : issue.severity === "warning"
                    ? "🟡"
                    : "🔵";
            stream.markdown(`${icon} **[${issue.rule_id ?? issue.severity}]** ${issue.message}` +
    ...
    if (d.rag_hits.length > 0) {
        ...
        for (const hit of d.rag_hits) {
            stream.markdown(`- **${hit.pattern}** *(similarity: ${hit.similarity.toFixed(2)})*\n` +
                `  ${hit.description}\n`);
        }
        stream.markdown("\n");
    }
